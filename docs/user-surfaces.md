...
| MCP server | `pnpm --filter @agent-pay/mcp-server` (stdio) | Exposes the [tools](../README.md#tools) (`check_x402_payment`, `verify_x402_settlement`, `get_payment_receipt`, `assess_subject`, `assess_account`, `payment_status`, `registry_status`, and the report tools) to any MCP-capable agent. Config sample on `/agents`. |
