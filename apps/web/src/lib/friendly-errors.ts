...
const REASON_COPY: Record<string, string> = {
  ...
  agent_pay_registry_package_hash_required:
    "AgentPay can't record decisions on Casper yet because its registry contract isn't configured.",
  ...
  agent_pay_registry_configuration_required:
    "AgentPay can't record decisions on Casper yet because its registry setup is incomplete.",
...
const SUBSTRING_COPY: Array<{ match: string; copy: string }> = [
  {
    ...
    copy: "AgentPay isn't set up to run live checks yet. The operator still needs to configure a Casper signing key."
  },
  {
    match: "AGENT_PAY_REGISTRY_PACKAGE_HASH",
    copy: "AgentPay can't record decisions on Casper yet because its registry contract isn't configured."
  },
  {
    match: "CASPER_RPC_URL",
    copy: "AgentPay has no Casper RPC endpoint configured, so it can't confirm on-chain activity."
  },
  {
